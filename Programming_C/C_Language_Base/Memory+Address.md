...
7. 함수 매개변수로 단일 포인터 변수를 선언할때.
	: 주소값의 복사본을 함수의 단일 포인터 변수가 받음. 그래서 함수 내부에서 주소값을 바꿔도 진짜 주소값은 변환X.

8. 진짜 주소값을 바꾸고 싶다면 딱 2가지만 가능. 	
	방법1: (Return 방식): 복사본 종이에 바뀐 주소를 적은 뒤, 함수가 끝날 때 메인 함수로 던져줘서 원본을 덮어쓰기 (main_head = delnode(main_head, target);)

	방법2:(더블 포인터 방식): 주소값 자체를 복사하지 말고, 원본 주소가 적힌 '방의 주소(&main_head)'를 넘겨서 함수 안에서 원본 방의 알맹이를 직접 파헤치기 (node head)

 	 Recap: 함수를 통해 메인 함수의 원본 데이터를 바꾸고 싶을 때는 "바꾸고 싶은 대상의 주소값(&)을 넘긴다."

	e.g.,)
	node*head가 가르키는 걸 바꾸려면 head의 주소를 인자로 해야하니깐 node**head를 줘야하네