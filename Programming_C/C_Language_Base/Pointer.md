...
0. 포인터 해석법:
	1>자료형 *p: p는 포인터.자료형의 주소를 저장하는 포인터.
	2>자료형 **pp: pp는 포인터. 자료형 *(자료형 포인터)의 주소를 저장하는 포인터.	
	//*변수에서 *변수 앞에 붙은 부분( 1>에선 자료형, 2>에선 자료형 *. ): 어떤 타입의 주소를 저장할까를 결정.
...
4. 포인터 연산 : 타입 크기 고려해서 +- 이동.
  1)	e.g.,) int a[5]= {10, 20 ,30 ,40 ,50};
		int *p=a; // p == &a[0]
		p+2; // >>&a[2], 즉 1바이트 이동이 아니라 4바이트씩 이동함.
		p++  // &a[3]; 
  2) 포인터 끼리 -+ 해서 배열의 길이 산출 가능. (단 연산하는 포인터는 같은 배열을 가르켜야함.)
	(이유: 포인터끼리의 뺄셈 (ptr1 - ptr2): 두 주소의 차이를 바이트 수로 반환하는 것이 아니라, 사이의 요소 개수를 반환. 즉 (주소 차이) / sizeof(T) 값 반환 해서.)

  3) 포인터 연산 계산 공식: 포인터 ptr이 타입 T를 가리키고 있을 때(T *ptr), ptr + N 또는 ptr - N 연산을 수행하면 실제 메모리 주소(Address)는 아래 공식으로 계산.
	
	공식: New address = base address+-(N*sizeof(T))
